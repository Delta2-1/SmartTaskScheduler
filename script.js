...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // === CONSOLE TRACKING LOGIC (ADDED) ===
    // ==========================================
    // This catches clicks on ANY button in the system
    document.addEventListener('click', (event) => {
        // Find the closest button element (in case inner spans/icons are clicked)
        const button = event.target.closest('button');

        if (button) {
            // Create a clean log object
            const logData = {
                text: button.innerText.replace(/\s+/g, ' ').trim(), // Clean whitespace
                id: button.id || 'N/A',
            ...
            console.group(`%c🖱️ BUTTON CLICK: ${logData.text}`, 'background: #222; color: #bada55; font-size: 12px; padding: 4px;');
            console.log('Button ID:', logData.id);
            console.log('Classes:', logData.classList);
            console.log('DOM Element:', button);
            console.groupEnd();
        }
    });
    // ==========================================
